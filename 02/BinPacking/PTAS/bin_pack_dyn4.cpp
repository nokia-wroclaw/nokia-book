...
static   int A[54][54][84][84];
  static int B[54][54][84][84];
  static X C[54][54][81][84];
  

dyn4::dyn4(vector<double> S, vector<int> x, double capacity)
{
  this->S = S;
  this->x = x;
...
void dyn4::calcPath(X cur, X pop)
{
  ...
  calcPath(sol, C[sol.a][sol.b][sol.c][sol.d]);
    ...
    bins.push_back(1.0 + 0.00000000001 - (S[0] * pop.a + S[1] * pop.b + S[2] * pop.c + S[3] * pop.d));

  //cout << sol.a << " " << sol.b << " " << sol.c << " " << sol.d << endl;
  DBG(cout << pop.a << " " << pop.b << " " << pop.c << " " << pop.d << endl;)
}
      ...
      for (int k = 0; k <= x[2]; k++)
        for (int l = 0; l <= x[3]; l++)
        {
          if (i + j + k + l == 0) continue;
            if (S[0] * 1.0 * i + S[1] * 1.0 * j + S[2] * 1.0 * k + S[3] * 1.0 * l <= capacity + 0.00000000001)
          {
            A[i][j][k][l] = 1;
            C[i][j][k][l] = {0,0,0,0};
...
vector<double> dyn4::dp()
{
  ...
  DBG(cout << cur.a << " " << cur.b << " " << cur.c << " " << cur.d << endl;
  cout << pop.a << " " << pop.b << " " << pop.c << " " << pop.d << endl;)

  while (1)
  { 
    X sol = {cur.a - pop.a, cur.b - pop.b,cur.c - pop.c,cur.d - pop.d};
    DBG(cout << sol.a << " " << sol.b << " " << sol.c << " " << sol.d << endl;)
